     ...
     */
        ...
        if(state =='l'){
            for (int i=0; i<l_leg.getNrOfSegments();i++){
                l_leg_fk_solver->JntToCart(l_jntarray, r_foot, i + 1);
...
int main(int argc, char **argv){
    ...
    ros::Subscriber sub_path=nh.subscribe<nav_msgs::Path>("humanoid/footstep_path",100,boost::bind(get_path,_1,sampling,&rate,&humanoid,&r_fksolver,&l_fksolver));
