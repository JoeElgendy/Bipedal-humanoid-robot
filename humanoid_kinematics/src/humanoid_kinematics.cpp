...
public:
    ...
    humanoid_kd(ros::NodeHandle *n, std::string urdf, std::string base, std::string Right_Leg_End_Effector, std::string Left_Leg_End_Effector,std::string Right_Arm_End_Effector, std::string Left_Arm_End_Effector)
    {
        ...
        else
        {
            ...
            std::cout << "lalala";
            //std::map<std::string,TreeElement>& segments = humanoid_tree.getSegments();
...
// joe: check this fun.
    ...
    bool humanoid_will_go_on_hab(char state, KDL::ChainFkSolverPos_recursive *Right_Leg_fk_solver, KDL::ChainFkSolverPos_recursive *Left_Leg_fk_solver,KDL::ChainFkSolverPos_recursive *Right_Arm_fk_solver,KDL::ChainFkSolverPos_recursive *Left_Arm_fk_solver ,ros::Rate rate, int freq = 200)
    {
        //sstd::cout<<"\nstate ="<<state<<"right leg ="<<Right_Leg_fk_solver<<"left leg = "<<Left_Leg_fk_solver<<"right arm = "<<Right_Arm_fk_solver<<"left arm = "<<Left_Arm_fk_solver<<"rate = "<<rate;
       // printf("\nstate =%c, right leg = %p, left leg = %p, right arm = %p, left arm = %p, size=%d\n", state, Right_Leg_fk_solver, Left_Leg_fk_solver, Right_Arm_fk_solver, Left_Arm_fk_solver, sizeof(KDL::ChainFkSolverPos_recursive));
        //"right leg ="<<Right_Leg_fk_solver<<""<<Left_Leg_fk_solver<<""<<Right_Arm_fk_solver<<""<<Left_Arm_fk_solver<<"rate = "<<rate;
        ...
        for (int k = 0; k < 14; k++)
        {
            ...
            if (P_x > P_limit) P_x = P_limit; else if (P_x < P_limit) P_x = -P_limit;//5aleha -p limit
...
void get_path(const nav_msgs::Path::ConstPtr &msg, int sampling, ros::Rate *rate, humanoid_kd *humanoid, KDL::ChainFkSolverPos_recursive *Right_Leg_fk_solver, KDL::ChainFkSolverPos_recursive *Left_Leg_fk_solver,KDL::ChainFkSolverPos_recursive *Right_Arm_fk_solver,KDL::ChainFkSolverPos_recursive *Left_Arm_fk_solver)
{
    ...
    for (int idx = 2; idx < msg->poses.size(); idx++)
    {
        ...
        //humanoid->humanoid_will_go_on(foot_stance, Right_Leg_fk_solver, Left_Leg_fk_solver,Right_Arm_fk_solver,Left_Arm_fk_solver, sampling * 1.5);
        if (foot_stance == 'r')
        {
...
int main(int argc, char **argv)
{
  ...
  //  std::cout << nh << "\n";
    ...
    humanoid_kd humanoid(&nh, urdf_file, "dummy", "Right_Foot_Link", "Left_foot_Link","Right_Lower_Shoulder_link","Left_Lower_Shoulder_Link");
